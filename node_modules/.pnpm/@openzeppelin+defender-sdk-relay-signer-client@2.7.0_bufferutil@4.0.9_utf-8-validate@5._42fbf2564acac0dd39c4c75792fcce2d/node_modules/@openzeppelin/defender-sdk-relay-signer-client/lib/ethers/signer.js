...
class DefenderRelaySigner extends ethers_1.JsonRpcSigner {
    constructor(relayerCredentials, provider, address, options = {}) {
    ...
    async signMessage(message) {
        ...
        const sig = await this.relayer.sign({
            message: (0, ethers_1.hexlify)(message),
        });
    ...
    async sendTransaction(transaction) {
        ...
        if ((0, utils_1.isLegacyTx)(tx) && tx.gasPrice !== undefined) {
            payloadGasParams = {
                gasPrice: (0, ethers_1.toBeHex)(tx.gasPrice),
            };
        }
        else if ((0, utils_1.isEIP1559Tx)(tx) && tx.maxFeePerGas !== undefined && tx.maxPriorityFeePerGas !== undefined) {
            payloadGasParams = {
                maxFeePerGas: (0, ethers_1.toBeHex)(tx.maxFeePerGas),
                maxPriorityFeePerGas: (0, ethers_1.toBeHex)(tx.maxPriorityFeePerGas),
            };
        }
        const payload = {
            to: tx.to?.toString(),
            gasLimit: (0, ethers_1.toBeHex)(tx.gasLimit),
            data: tx.data ? (0, ethers_1.toBeHex)(tx.data) : undefined,
            speed: tx.speed,
            value: tx.value ? (0, ethers_1.toBeHex)(tx.value) : undefined,
        ...
        let gasParams;
        if ((0, utils_1.isEIP1559Tx)(relayedTransaction)) {
            gasParams = {
        ...
        return this.provider._wrapTransactionResponse({
    ...
    async populateTransaction(transaction) {
        const tx = await (0, ethers_1.resolveProperties)(this.checkTransaction(transaction));
        if (tx.to != null) {
            // relayer provider acts as name resolver if parameter is an ENS name
            tx.to = await (0, ethers_1.resolveAddress)(tx.to, this.provider);
        }
        if (tx.gasLimit == null) {
            tx.gasLimit = await this.estimateGas(tx).catch((error) => {
    ...
    async _signTypedData(domain, types, value) {
        const domainSeparator = ethers_1.TypedDataEncoder.hashDomain(domain);
        const hashStructMessage = ethers_1.TypedDataEncoder.from(types).hash(value);
        const sig = await this.relayer.signTypedData({
            domainSeparator: (0, ethers_1.hexlify)(domainSeparator),
            hashStructMessage: (0, ethers_1.hexlify)(hashStructMessage),
        });
        return ethers_1.Signature.from(sig).serialized;
    }
}
